...
const AddAlarmModal = ({ visible, onClose, onAdd, alarmToEdit, onEdit }) => {
  ...
  useEffect(() => {
    if (visible) {
      if (isEditing) {
        ...
        setSelectedDays(alarmToEdit.repeat || []);
        setIsSpecificDate(alarmToEdit.isSpecificDate || false);
        setSelectedDate(alarmToEdit.specificDate || '');
      } else {
        // 为新闹钟设置默认值
        const now = new Date();
        ...
        setLabel('');
        setSelectedDays(weekDays.map(d => d.value)); // 默认选中每天
        setIsSpecificDate(false);
        setSelectedDate('');
      }
    }
  }, [visible, alarmToEdit, isEditing]);
